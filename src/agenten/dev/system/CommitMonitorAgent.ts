...
interface CommitVorschlag {
  summary: string;
  body: string;
}

const CommitMonitorAgent = {
  async run(): Promise<CommitVorschlag> {
  ...
  async buildCommitText(): Promise<CommitVorschlag> {
    let gitstatusRaw = "";
    ...
    try {
      ...
      const gitstatus: GitStatus = JSON.parse(gitstatusRaw);
  ...
  saveAsMarkdown({ summary, body }: CommitVorschlag) {
